/// Shroud — Stealth & OPSEC Utilities
///
/// Provides randomized User-Agent selection and jitter delays
/// to reduce detection risk during network reconnaissance.
///
...
/// Sleeps for a random duration between 10ms and max_ms to add request jitter.
pub async fn jitter(max_ms: u64) {
    if max_ms == 0 {
        return;
    }
    let delay = rand::thread_rng().gen_range(10..=max_ms);
